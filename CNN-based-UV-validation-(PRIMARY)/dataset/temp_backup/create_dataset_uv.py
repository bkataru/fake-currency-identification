...
def modify_image(img_name, inverted_flag, ot_flag, dataset_type):
...
# 3 * 1000 * 2 + 3 * 500 * 2
total = (len(folders) * training_count * 2) + (len(folders) * test_count * 2)
total += 100
def main(dataset_type):
    for fold in folders:
            ...
            for img_ind in range(0, len(img_list)):
                ...
                with progressbar.ProgressBar(max_value=total) as bar:
                    for iter in range(1, iterations + 1):
                        image = modify_image('.\\comp_data\\' + fold + '\\' + img_list[img_ind], inverted_flag, ot_flag, dataset_type)
                        if inverted_flag:
                            cv2.imwrite('.\\{}\\{}\\uvdown-{}-{}-{}.jpg'.format(dataset_type, fold, id, iter, UNIQUE), image)
                        else:
                            cv2.imwrite('.\\{}\\{}\\uvup-{}-{}-{}.jpg'.format(dataset_type, fold, id, iter, UNIQUE), image)
                    ...
                    if rem != 0:
                        for rem_iter in range(1, rem + 1):
                            image = modify_image('.\\comp_data\\' + fold + '\\' + img_list[0], inverted_flag, ot_flag, dataset_type)
                            if inverted_flag:
                                cv2.imwrite('.\\{}\\{}\\uvdown-{}-{}-rem.jpg'.format(dataset_type, fold, id, rem_iter), image)
                            else:
                                cv2.imwrite('.\\{}\\{}\\uvup-{}-{}-rem.jpg'.format(dataset_type, fold, id, rem_iter), image)
        ...
        if dataset_type == 'training_set':
            generate_images(files, 1000, "UV", False)
            generate_images(files, 1000, "UV", True)
        elif dataset_type == 'test_set':
            generate_images(files, 500, "UV", False)
            generate_images(files, 500, "UV", True)
